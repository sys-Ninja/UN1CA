...
if [ -d "$MODPATH/smali_framework" ]; then
    LOG "- Adding VoiceChanger audio interceptor to /system/framework/framework.jar"
    mkdir -p "$APKTOOL_DIR/system/framework/framework.jar/smali_classes6/io/mesalabs/unica/audio"
    cp -a "$MODPATH/smali_framework/"* "$APKTOOL_DIR/system/framework/framework.jar/smali_classes6/io/mesalabs/unica/audio/"
fi

# Hook AudioRecord for system-wide voice changing (all apps)
