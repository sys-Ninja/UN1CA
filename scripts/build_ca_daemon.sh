...
#
# Cross-compiles the AI call answering daemon for arm64 and drops the binary
...
MODULE_DIR="$SRC_DIR/target/a52sxq/patches/01_callai"
API_LEVEL=31
...
echo "NDK:    $NDK"
echo "Output: $MODULE_DIR/system/bin/ca_daemon"

mkdir -p "$MODULE_DIR/system/bin"
"$CC" -O2 -Wall -Wextra \
    -o "$MODULE_DIR/system/bin/ca_daemon" \
    "$MODULE_DIR/src/ca_daemon.c" \
    -ldl -lm -llog

"$NDK/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-strip" \
    "$MODULE_DIR/system/bin/ca_daemon"

file "$MODULE_DIR/system/bin/ca_daemon"
